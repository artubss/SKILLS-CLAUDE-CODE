...
description: Orquestre workflows de automação complexos com dependências de tarefas, agendamento e execução multiplataforma
---

# Workflow Orchestrator

Orquestre workflows de automação complexos: $ARGUMENTS

## Estado Atual do Workflow

- Workflows existentes: !`find . -name "*.workflow.json" -o -name "workflow.yml" -o -name "Taskfile.yml" | head -5`
- Jobs cron: !`crontab -l 2>/dev/null || echo "No crontab found"`
- Processos em execução: !`ps aux | grep -E "(workflow|task|job)" | head -3`
- Capacidades do sistema: !`which docker node python3 | head -3`
- Configuração: @.workflow-config.json ou @workflows/ (se existir)

## Tarefa

Criar e gerenciar workflows de automação complexos com gerenciamento de dependências, agendamento e monitoramento.

## Estrutura de Definição de Workflow

### Schema Básico de Workflow
```json
{
...
### 2. **Execução Paralela de Tarefas**
```json
{
  "id": "parallel-tests",
...
### 4. **Processamento de Arquivos e Dados**
```json
{
  "id": "process-data",
...
## Motor de Orquestração de Workflow
...
### Implementação de Tipos de Tarefas
...
Este orquestrador de workflow oferece capacidades de automação em nível empresarial com gerenciamento de dependências, monitoramento e suporte à execução multiplataforma.