...
description: Gerenciar e automatizar configuração de pipeline CI/CD com GitHub Actions, suporte a multi-ambiente e estratégias de deploy
---

# Gerenciador de Pipeline CI/CD

Gerencie automação de pipeline CI/CD: $ARGUMENTS

## Estado Atual do Pipeline

- GitHub Actions: !`find .github/workflows -name "*.yml" -o -name "*.yaml" 2>/dev/null | head -5`
- Configuração de CI: @.github/workflows/ (se existir)
- Scripts de pacote: @package.json
- Arquivos de ambiente: !`find . -name ".env*" | head -3`
- Execuções recentes de workflow: !`gh run list --limit 5 2>/dev/null || echo "GitHub CLI not available"`

## Tarefa

Automatize o gerenciamento de pipeline CI/CD com orquestração abrangente de workflow.

## Operações do Pipeline

### Configurar Novo Pipeline
Crie pipeline CI/CD completo com:
...
### Portais de Segurança e Qualidade
```yaml
  security-scan:
    runs-on: ubuntu-latest
    steps:
...
### 1. **Testes com Estratégia de Matrix**
```yaml
strategy:
  matrix:
...
# Visualizar execução específica
...
#### 1. **Problemas de Permissão de Workflow**
```yaml
permissions:
  contents: read
  actions: write
...
```bash
# Adicionar secret do repositório
...
### 1. **Estratégia Falhe Rápido**
- Execute tarefas mais rápidas primeiro
- Use `fail-fast: true` em matriz
- Implemente etapas de validação antecipadas

### 2. **Segurança em Primeiro Lugar**
- Nunca armazene secrets no código
- Use permissões de menor privilégio
- Escaneie vulnerabilidades antecipadamente

### 3. **Otimização de Eficiência**
- Use estratégias de cache apropriadas
- Minimize duração do workflow
- Execução paralela de jobs

### 4. **Monitoramento e Alertas**
- Rastreie taxas de sucesso de build
- Monitore frequência de deployment
...
Este gerenciador de pipeline fornece automação abrangente para workflows CI/CD modernos com segurança, performance e monitoramento integrados.