...
The **Vapi documentation MCP server** (`vapi-docs`) provides the full Vapi knowledge base (for example `searchDocs` when that MCP is connected).

**Configure MCP** in this repository (for example `.mcp.json` for Claude Code). If it is not already present, run:
